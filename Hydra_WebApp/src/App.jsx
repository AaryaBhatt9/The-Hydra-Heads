/**
 * Hydra Heads Recovery Intelligence — GlobeHack Season 1
 *
 * Setup:
 *   npm create vite@latest hydra-heads-demo -- --template react
 *   cd hydra-heads-demo && npm install
...
${asmt.visionFlaggedAreas?.length?`  Vision flagged areas      : ${asmt.visionFlaggedAreas.join(", ")}`:""}` : "No camera assessment.";
  ...
  const prompt = `You are a Hydra Heads wellness protocol specialist. Generate a personalized Hydra Heads session protocol using ONLY wellness language.
...
DEVICE: Hydra Heads dual-pad system
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: "text",
            text: `You are a Hydra Heads wellness specialist analyzing a patient posture photo.
...
const downloadReportPdf = () => {
  ...
  pdf.setFontSize(13);pdf.setFont("helvetica","bold");pdf.setTextColor(255,255,255);pdf.text("HYDRA HEADS",margin,11);
  ...
  for(let i=1;i<=tp;i++){pdf.setPage(i);pdf.setFillColor(245,247,250);pdf.rect(0,285,W,12,"F");pdf.setFontSize(8);pdf.setFont("helvetica","normal");pdf.setTextColor(140,140,160);pdf.text("Hydra Heads · Recovery Intelligence · GlobeHack S1 · Wellness use only",margin,292);pdf.text(`Page ${i} of ${tp}`,W-margin,292,{align:"right"});}
  pdf.save(`${r.merged.patient.name||"hydra-heads"}-report.pdf`);
};
...
const generateReport = () => {
  ...
  return (
    <div style={C.root}>
      ...
      <div style={C.hdr}>
        <div>
          <div style={{fontSize:18,fontWeight:500}}>Hydra Heads</div>
        ...
        <div>
          ...
          <div style={{...C.sec,marginBottom:"1.25rem",padding:"0.875rem 1.25rem"}}>
            <div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"var(--color-text-secondary)",lineHeight:"1.9"}}>
              ...
              → Claude API → Protocol → MQTT → Hydra Heads device
            </div>
          </div>
